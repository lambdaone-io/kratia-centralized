...
import lambdaone.kratia.collector.{BallotBox, InfluenceAllocation, Timestamp}
...
object CrudPickSqlCollector {
  ...
  def apply[F[_]: Monad, A: Read: Put, D: Read: Put](
  ...
  ) = new CRUDPick[F, A, BoxData] {

    /** Stores `a` with the chosen new unique reference */
    override def create(boxData: BoxData, id: A): F[A] = {
    ...
    override def get(id: A): F[Option[BoxData]] = {
      val ballotData: F[Option[BoxData]] =
        sql"select closes_on, data from ballotbox_binary where id=${id}"
          .query[(Timestamp, D)]
          .map {
            case (ts, d) =>
              BoxData(BinaryProposal.ballot, ts, d, Map())
          }
          .option
          .transact(xa)
    ...
    override def update(id: A)(
      ...
      val before: F[Option[BoxData]] = get(id)
      val after: F[Option[BoxData]] = before.map(_.map(f))

      val xx: F[Option[F[List[Int]]]] = (before, after).mapN { (optB, optA) =>
        (optB, optA).mapN { (b, a) =>
          val addedVotes
            : Set[(A, (A, InfluenceAllocation))] = a.votes.toSet diff b.votes.toSet
          val removedVotes = b.votes.toSet diff a.votes.toSet

          val added: Set[doobie.ConnectionIO[Int]] = addedVotes.map {
            case (member, (proof, InfluenceAllocation(ia))) =>
              val yes: Double = ia.getOrElse(BinaryProposal.Yes, 0.0)
              val no: Double = ia.getOrElse(BinaryProposal.No, 0.0)
              sql"insert into votes_binary (box, member, proof, yes, no) values ($id, ${member}, ${proof}, ${yes}, ${no})".update.run
          }
    ...
    override def delete(id: A): F[Option[BoxData]] =
    ...
    override def all: F[Map[A, BoxData]] = {
