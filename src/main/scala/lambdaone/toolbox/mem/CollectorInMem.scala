...
case class CollectorInMem[I, A](gen: UniqueGen[IO, I]) extends Collector[InMem[I, A, ?], I, A] {

  private def withRef[T](f: Ref[IO, Map[I, A]] => IO[T]): InMem[I, A, T] =
    Kleisli.ask[IO, Imports[I, A]].flatMapF(f)

  override def create(ballot: Collector.Ballot[A], nickname: String): InMem[I, A, Collector.BallotBox[I, A]] = ???

  override def vote(ballotBox: Collector.BallotBox[I, A], vote: Collector.Vote[I, A]): InMem[I, A, Collector.ProofOfVote[I]] = ???

  override def validateVote(proofOfVote: Collector.ProofOfVote[I]): InMem[I, A, Boolean] = ???

  override def inspect(ballotBox: Collector.BallotBox[I, A]): InMem[I, A, InfluenceAllocation[A]] = ???
